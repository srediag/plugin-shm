...
func init() {
	var err error
	asyncCallbackPool, err = ants.NewPool(128) // pool size can be tuned
	if err != nil {
		panic("failed to create ants pool: " + err.Error())
	}
}
...
func (s *Stream) fillDataToReadBuffer(buf bufferSliceWrapper) error {
	...
	if callback != nil {
		...
		if atomic.CompareAndSwapUint32(&s.callbackInProcess, 0, 1) {
			...
			err := asyncCallbackPool.Submit(func() {
				for {
					s.pendingData.moveTo(s.recvBuf)
			...
			if err != nil {
				// fallback: if pool is full, run in a new goroutine (to avoid deadlock)
				go func() {
					for {
						s.pendingData.moveTo(s.recvBuf)
						for s.IsOpen() && s.recvBuf.Len() > 0 {
...
func (r *pendingData) moveToWithoutLock(toBuf *linkedBuffer) {
	...
	for i := range r.unread {
		...
		// }
		// In heap-only mode, shared memory not supported, skip
		continue
	}
