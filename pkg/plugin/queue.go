...
import (
	...
	cqueue "github.com/Workiva/go-datastructures/queue"
)
...
func mappingQueueManagerMemfd(queuePathName string, memFd int) (*queueManager, error) {
	...
	return &queueManager{
		sendQueue:   mappingQueueFromBytes(),
		recvQueue:   mappingQueueFromBytes(),
		mem:         mem,
		path:        queuePathName,
		memFd:       memFd,
		mmapMapType: MemMapTypeMemFd,
...
func createQueueFromBytes(_ []byte, cap uint32) *queue {
	return &queue{q: cqueue.New(int64(cap))}
}

func mappingQueueFromBytes() *queue {
	return &queue{q: cqueue.New(0)}
}
